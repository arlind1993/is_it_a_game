...
Future<void> guardWithCrashlytics(
  ...
  await runZonedGuarded<Future<void>>(() async {
    ...
    Logger.root.onRecord.listen((record) {
      ...
      LoggerPrinter.printLogFrom(record.level, record.loggerName, record.message, recordedTime: record.time);
    ...
    LoggerPrinter.printLogFrom(Level.WARNING, "guardWithCrashlytics", error.toString(), stacktrace: stack.toString());
...
StackTrace filterStackTrace(StackTrace stackTrace) {
  try {
    ...
    for (final line in lines) {
      if (line.contains('firebase_crash.dart') ||
